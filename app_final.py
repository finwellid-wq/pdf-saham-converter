                        ...
                        try:
                            if len(row) >= 9:
                                ...
                                if transaction_type and jumlah_saham and harga and tanggal:
                                    trans = {'saham': metadata.get('saham', 'UNKNOWN'), 'date': tanggal, 'broker': 'Penjualan' if 'Penjualan' in transaction_type else 'Pembelian', 'big_player': metadata.get('big_player', 'Unknown'), 'jumlah_sebelum': 0, 'change': -jumlah_saham if 'Penjualan' in transaction_type else jumlah_saham, 'jumlah_sesudah': 0, 'harga': harga}
...
def process_pdfs():
    try:
        ...
        if not all_transactions:
            return jsonify({'success': False, 'error': 'No transactions found in PDFs'}), 400
